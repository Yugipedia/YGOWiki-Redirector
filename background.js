// Simple extension to redirect all requests to YGO Wikia to Yugipedia
(function(){
  'use strict';
  ...
  const WIKIA_REGEX = /^(ygo|yu-gi-oh|yugioh)\.(wikia|fandom)\.com$/i; // Used to match the domain of the old wikia/fandom to make sure we are redirecting the correct domain.
  ...
  chrome.webNavigation.onBeforeNavigate.addListener(
    function(info) {
      if(isPluginDisabled) { // Ignore all navigation requests when the extension is disabled.
        console.log("YGOWikia intercepted, ignoring because plugin is disabled.");
        return;
      }
      ...
      // Generate new url
      const host = 'yugipedia';
      const redirectUrl = `https://${host}.com${url.pathname}`; // Create the redirect URL
      console.log(`YGOWikia intercepted:  ${info.url}\nRedirecting to ${redirectUrl}`); 
  ...
  storage.onChanged.addListener(
      function(changes, areaName) {
        ...
        if(changes["isDisabled"]!==undefined && changes["isDisabled"].newValue!=changes["isDisabled"].oldValue) {
          console.log(`YGOWikia Redirector is now ${changes["isDisabled"].newValue?'disabled':'enabled'}`);
